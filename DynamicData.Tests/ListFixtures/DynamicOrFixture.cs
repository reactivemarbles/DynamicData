...
namespace DynamicData.Tests.ListFixtures
{
    ...
    public class DynamicOrFixture
    {
        ...
        [Test]
        public void DynamicOrShouldWork()
        {
            var a = new SourceList<int>();
            var b = new SourceList<int>();
            var c = new SourceList<int>();
            ...
            CollectionAssert.AreEquivalent( new [] { 1,3 },target.Data.Items);
            _source.Add(c);
            // In this case the duplicate 1 should only occur once.
            Assert.AreEqual(target.Data.Items.Count(), 3);
            CollectionAssert.AreEquivalent( new [] { 1,3,5 },target.Data.Items);

            // It requires removing 1 from both c and a before it 
            // disapears from the resul
            _source.Remove(c);
            CollectionAssert.AreEquivalent( new [] { 1,3 },target.Data.Items);
            _source.Remove(a);
            CollectionAssert.AreEquivalent( new [] { 3 },target.Data.Items);
