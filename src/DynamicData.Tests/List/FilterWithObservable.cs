...
{
    private readonly ReactiveUI.Primitives.Signals.StateSignal<Func<Person, bool>> _filter;
    ...
    [Test]
    public async Task ChainFilters()
    {
        var filter2 = new ReactiveUI.Primitives.Signals.StateSignal<Func<Person, bool>>(person1 => person1.Age > 20);
        ...
        await Assert.That(captureList).IsEquivalentTo(new[] { 1, 0 });
    }
    ...
    public async Task ReevaluateFilter()
    {
        ...
        await Assert.That(_results.Data.Count).IsEqualTo(90).Because("Should be 90 people in the cache");
        await Assert.That(_results.Messages.Count).IsEqualTo(2).Because("Should be 2 update messages");
        await Assert.That(_results.Messages[1].Removes).IsEqualTo(0).Because("Should be 80 removes in the second message");
        await Assert.That(_results.Messages[1].Adds).IsEqualTo(10).Because("Should be 10 adds in the second message");
    ...
    public async Task RemoveFiltered()
    {
        ...
        _source.Add(person);
        await Assert.That(_results.Data.Count).IsEqualTo(0).Because("Should be 0 people in the cache");
        _filter.OnNext(p => p.Age >= 1);
        await Assert.That(_results.Data.Count).IsEqualTo(1).Because("Should be 1 people in the cache");
    ...
    [Test]
    public async Task RemoveFilteredRange()
    {
        ...
        await Assert.That(_results.Data.Count).IsEqualTo(5).Because("Should be 5 people in the cache");
