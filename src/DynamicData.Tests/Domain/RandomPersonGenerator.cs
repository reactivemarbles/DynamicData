...
public class RandomPersonGenerator
{
    ...
    {
        var girls = (from first in _girls from second in _lastnames from third in _lastnames select new { First = first, Second = second, Third = third, Gender = "F" });

        var boys = (from first in _boys from second in _lastnames from third in _lastnames select new { First = first, Second = second, Third = third, Gender = "M" });

        var maxage = 100;
        return girls.Union(boys).OrderBy(x => Guid.NewGuid()).Select(
            x =>
            {
