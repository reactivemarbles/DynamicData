    ...
    where TObject : INotifyPropertyChanged
{
    ...
    // Pending notifications are dequeued FIFO across both sub-queues. Enqueues on the drainer
    // thread can drain reentrantly, so chain processing and user delivery can nest. Neither
    // the chain walk nor downstream callbacks run under the shared queue's lock.
    //
