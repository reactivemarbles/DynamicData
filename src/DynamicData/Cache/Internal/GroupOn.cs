    ...
    where TGroupKey : notnull
{
    ...
    public IObservable<IGroupChangeSet<TObject, TKey, TGroupKey>> Run() => Observable.Create<IGroupChangeSet<TObject, TKey, TGroupKey>>(
            observer =>
            {
                ...
                var groups = _source.SynchronizeSafe(queue).Finally(observer.OnCompleted).Map(grouper.Update).Keep(changes => changes.Count != 0);

                var regroup = _regrouper.SynchronizeSafe(queue).Map(_ => grouper.Regroup()).Keep(changes => changes.Count != 0);
