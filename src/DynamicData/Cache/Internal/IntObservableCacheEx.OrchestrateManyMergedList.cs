...
internal static partial class IntObservableCacheEx
{
    ...
    public static IObservable<IChangeSet<TDest>> OrchestrateManyMergedList<TSource, TKey, TDest>(
        ...
        where TKey : notnull
        where TDest : notnull =>
        source.OrchestrateMany<TSource, TKey, IChangeSet<TDest>, IChangeSet<TDest>, MergedListOrchestrator<TSource, TKey, TDest>>(
            (context, emitter) => new MergedListOrchestrator<TSource, TKey, TDest>(context, emitter, changeSetSelector, equalityComparer));

    internal sealed class MergedListOrchestrator<TSource, TKey, TDest> : OrchestratorCacheChangeBase<TSource, TKey, IChangeSet<TDest>, IChangeSet<TDest>>
        where TSource : notnull
        where TKey : notnull
        where TDest : notnull
    {
        ...
        public MergedListOrchestrator(
                ICacheOrchestratorContext<TKey, IChangeSet<TDest>> context,
                IObserver<IChangeSet<TDest>> emitter,
                Func<TSource, TKey, IObservable<IChangeSet<TDest>>> changeSetSelector,
