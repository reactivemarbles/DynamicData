...
{
    /// <summary>
    /// Provides an overload of <c>FullJoin</c> for the supplied arguments.
    /// </summary>
    ...
    /// <remarks>Overload that omits the key from the result selector. Delegates to <c>FullJoin&lt;TLeft, TLeftKey, TRight, TRightKey, TDestination&gt;(IObservable&lt;IChangeSet&lt;TLeft, TLeftKey&gt;&gt;, IObservable&lt;IChangeSet&lt;TRight, TRightKey&gt;&gt;, Func&lt;TRight, TLeftKey&gt;, Func&lt;TLeftKey, Optional&lt;TLeft&gt;, Optional&lt;TRight&gt;, TDestination&gt;)</c>.</remarks>
    ...
    /// Both sides are <c>Optional&lt;T&gt;</c> because a given key may only exist on one side at any point.
    ...
    /// <param name="right">The right <c>IObservable&lt;IChangeSet&lt;TRight, TRightKey&gt;&gt;</c> to join.</param>
    /// <param name="rightKeySelector">A <c>Func&lt;T, TResult&gt;</c> that maps each right item to the left key it should join on.</param>
    /// <param name="resultSelector">A <c>Func&lt;T, TResult&gt;</c> that combines the key, optional left, and optional right into a destination object. Example: <c>(key, left, right) =&gt; new Result(key, left, right)</c>.</param>
    ...
    ///   <item><term>Add</term><description>Emits with the left value and the matching right (or <c>ReactiveUI.Primitives.Optional.None&lt;T&gt;</c> if no right exists).</description></item>
    ...
    ///   <item><term>Remove</term><description>If a right match still exists, re-invokes the selector with left as <c>ReactiveUI.Primitives.Optional.None&lt;T&gt;</c>. If neither side remains, removes the joined result.</description></item>
    ...
    /// <list type="table">
    ///   <listheader><term>Event</term><description>Behavior</description></listheader>
    ///   <item><term>Add</term><description>Emits with the matching left (or <c>ReactiveUI.Primitives.Optional.None&lt;T&gt;</c>) and the right value.</description></item>
    ///   <item><term>Update</term><description>Re-invokes selector with current left (if any) and the new right value.</description></item>
    ///   <item><term>Remove</term><description>If a left match still exists, re-invokes the selector with right as <c>ReactiveUI.Primitives.Optional.None&lt;T&gt;</c>. If neither side remains, removes the joined result.</description></item>
    ///   <item><term>Refresh</term><description>Forwarded as Refresh on the joined result.</description></item>
    /// </list>
    /// </para>
