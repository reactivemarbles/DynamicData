...
{
    /// <summary>
    /// Provides an overload of <c>FullJoinMany</c> for the supplied arguments.
    /// </summary>
    ...
    /// <param name="resultSelector">A <c>Func&lt;T, TResult&gt;</c> that combines the optional left value and the right group into a destination object. The key is not provided in this overload.</param>
    /// <returns>The resulting observable sequence.</returns>
    /// <remarks>Overload that omits the key from the result selector. Delegates to <c>FullJoinMany&lt;TLeft, TLeftKey, TRight, TRightKey, TDestination&gt;(IObservable&lt;IChangeSet&lt;TLeft, TLeftKey&gt;&gt;, IObservable&lt;IChangeSet&lt;TRight, TRightKey&gt;&gt;, Func&lt;TRight, TLeftKey&gt;, Func&lt;TLeftKey, Optional&lt;TLeft&gt;, IGrouping&lt;TRight, TRightKey, TLeftKey&gt;, TDestination&gt;)</c>.</remarks>
    public static IObservable<IChangeSet<TDestination, TLeftKey>> FullJoinMany<TLeft, TLeftKey, TRight, TRightKey, TDestination>(this IObservable<IChangeSet<TLeft, TLeftKey>> left, IObservable<IChangeSet<TRight, TRightKey>> right, Func<TRight, TLeftKey> rightKeySelector, Func<ReactiveUI.Primitives.Optional<TLeft>, IGrouping<TRight, TRightKey, TLeftKey>, TDestination> resultSelector)
        where TLeft : notnull
    ...
    /// <c>Optional&lt;T&gt;</c> because only the right side may have entries for a given key.
    ...
    /// <param name="right">The right <c>IObservable&lt;IChangeSet&lt;TRight, TRightKey&gt;&gt;</c> to join.</param>
    /// <param name="rightKeySelector">A <c>Func&lt;T, TResult&gt;</c> that maps each right item to the left key it should join on.</param>
    /// <param name="resultSelector">A <c>Func&lt;T, TResult&gt;</c> that combines the key, optional left value, and the right group into a destination object. Example: <c>(key, left, group) =&gt; new Result(key, left, group)</c>.</param>
    ...
    ///   <item><term>Remove</term><description>If the right group is non-empty, re-invokes with left as <c>ReactiveUI.Primitives.Optional.None&lt;T&gt;</c>. If both sides are empty, removes the result.</description></item>
    ...
    public static IObservable<IChangeSet<TDestination, TLeftKey>> FullJoinMany<TLeft, TLeftKey, TRight, TRightKey, TDestination>(this IObservable<IChangeSet<TLeft, TLeftKey>> left, IObservable<IChangeSet<TRight, TRightKey>> right, Func<TRight, TLeftKey> rightKeySelector, Func<TLeftKey, ReactiveUI.Primitives.Optional<TLeft>, IGrouping<TRight, TRightKey, TLeftKey>, TDestination> resultSelector)
        where TLeft : notnull
        where TLeftKey : notnull
        where TRight : notnull
        where TRightKey : notnull
